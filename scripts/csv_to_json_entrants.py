...
def parse_csv_to_json():
    # File paths
    csv_file = Path("/home/jemmal/dev/ormt/ormt/ormt-api/data/back/nombre d'entrp[rises entrants.csv")
    json_file = Path("/home/jemmal/dev/ormt/ormt/ormt-api/data/init-data/domaines/demande-de-travail/sous-domaines/tissu-economique/data/nombre d'entreprises entrants.json")
    ...
    for row_idx in range(1, len(rows)):
        ...
        for sector_idx, sector in enumerate(sector_columns):
            value_col_idx = sector_idx + 3  # Offset by 3 for annuel, taille, region
    ...
    json_data = {
        "indicateur": "nombre d'entreprises entrants",
        "data": data_entries
    }
